...
public class MenuController {
    ...
    private void handleMenuChange(int dir) {
        ...
        if (currentPage == 1) {
            ...
            else if (sel == 2 && p.advancedGrainExperimental == 0) p.grain = Math.max(0, Math.min(5, p.grain + dir));
            else if (sel == 3 && p.advancedGrainExperimental == 0 && p.grain > 0) p.grainSize = Math.max(0, Math.min(2, p.grainSize + dir));
            else if (sel == 4) p.vignette = Math.max(0, Math.min(5, p.vignette + dir));
            else if (sel == 5) p.advancedGrainExperimental = p.advancedGrainExperimental == 0 ? 1 : 0;
    ...
    private void render() {
        ...
        if (currentMainTab == 0) {
            if (currentPage == 1) {
                ...
                setRow(5, "Advanced Grain (Experimental)", p.advancedGrainExperimental != 0 ? "ON" : "OFF");
            } else if (currentPage == 5) {
                ic = 6;
    ...
    private boolean isRowActive(RTLProfile p, int i) {
        ...
        if (currentMainTab == 0 && currentPage == 4) {
            ...
            if (i == 2 || i == 3) return p.advancedGrainExperimental == 0 && (i != 3 || p.grain > 0);
        }
        return true;
    }
